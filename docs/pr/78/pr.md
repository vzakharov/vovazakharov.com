...
- [ ] `ci-cold-cache` — the first deploy after merge renders the full set inside the budget, and `findChromium()` resolves against whatever the runner ships.
- [ ] `ci-warm-cache` — a later deploy with one document changed reprints only that document.
...
| `ci-cold-cache`  | only in CI  | no                             | Observed on the first real deploy. Chromium on the runner is load-bearing now.    |
| `ci-warm-cache`  | only in CI  | no                             | Observed on the second.                                                           |
...
- **T01** `.github/actions/render-pdfs/action.yml`:1 — unresolved — last: @vzakharov (human) 2026-09-22T13:16:16Z — "как мы можем проверить, что оно всё работает, до мёрджа?" → [↓](#t01)
- **T02** `.github/actions/render-pdfs/action.yml`:1 — unresolved — last: @vzakharov (human) 2026-09-22T13:17:13Z — "и второй вопрос -- мы параллелим рендер?" → [↓](#t02)
- **T03** `scripts/render-pdf.ts`:16 — unresolved — last: @vzakharov (human) 2026-09-22T13:21:39Z — "на всякий случай перепроверь на медведей едё раз (здесь и ан…" → [↓](#t03)
- **T04** `.gitattributes`:4 — unresolved — last: @vzakharov (human) 2026-09-22T13:52:16Z — "что за рендеры которые stay committed?" → [↓](#t04)
