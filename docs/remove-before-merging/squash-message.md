...
```
ci: render page PDFs in CI instead of committing them (pr #78)
```
...
A ci: subject now publishes, alongside feat:, fix: and perf:: a change
to the pipeline is tested only by a deploy, and one the gate skipped
was first run inside whatever release landed next. The publishing set
is stated once, in the gate beside each type's reason, and CLAUDE.md
points at it rather than listing it.
