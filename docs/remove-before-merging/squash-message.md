...
`scripts/check-squash-message.sh` settles the part of the
squash-message target a machine can. `/squash-message` states it as
prose — a one-line title, a body hard-wrapped at ~72 — and its
tighten pass is an agent reading its own output; the script measures
the proposal's two fenced blocks against 80 chars of title and 50
lines of body at 72 wide. It joins the vet fan-out as its tenth check,
reading only the proposal, so it catches what authorship cannot: a
