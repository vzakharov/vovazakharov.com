...
vet runs and fails on whenever it had to rewrite one. A CSS module's
class set is unknown to the type system, so every lookup is keyed —
classes['card'] — which is what noPropertyAccessFromIndexSignature asks
for. CVPage and HomePage split along the seams the conversion exposed,
which moves the root page's English-copy lint exemption from one file to
the whole slice; useMounted gives way to the @mantine/hooks one.
