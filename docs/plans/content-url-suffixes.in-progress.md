...
**The prefix is a legibility trade, and a small one.** Neither form is a URL: a dot already means three other things in these names — the host's own, the cut suffix, the extension — so no parser recovers a path from either, and both rely on a reader recognising the name. Spelling the host in full would only put a resolvable string _inside_ the filename, to be copied out by hand; `vova` gives up that much and buys a name legible at a glance. The PDF gives up nothing at all, carrying its canonical URL in the footer. One `SITE_CONFIG` field is the whole of the decision, so it reverses in a line if the short form ever reads as too anonymous.
