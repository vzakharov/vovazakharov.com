...
- Two hotfixes are candid about production defects worth thinking about before quoting: `hotfix: #2239 stop impersonated sessions from consuming the target's tips` (a superuser impersonation billing bug) and `hotfix: #2165 temporarily disable deep research while its backend is rebuilt` (a shipped feature switched off for ~9 days).
