...
This supersedes the spinoff plan on `claude/spinoff-phnz8v` (PR #44). Nothing
leaves this repository, so `/spinoff` is not run and PR #43 lands unchanged:
`writing/late-stage-agentic/` arrives on `main` as drafts, and a later PR turns
those drafts into the pages the LSA site serves.
...
- **It does not move any content.** `writing/late-stage-agentic/` stays drafts. Turning them into the pages LSA serves is a later PR, after PR #43 lands and after the video is recorded.
- **It does not touch the four dictation skills or `scripts/transcribe.py`** — they are already going to `main` on PR #43 and are unaffected.
...
1. **What does the LSA site serve on day one?** (a) A real index page with no
   ...
   content pipeline pointed at a new collection and PR #43 landed first, so it
   makes this plan wait on both.
...
3. **When does this land relative to PR #43?** (a) Base off `main` now, merge
   after #43 — recommended; they overlap only in CLAUDE.md, which is a small
   conflict to resolve once. (b) Wait for #43 to land first and start from there,
   which costs nothing but time if #43 sits.
4. **Is PR #44 closed as superseded?** (a) Yes — recommended, and done as part of
   publishing this plan, with a comment pointing here. (b) Left open.
