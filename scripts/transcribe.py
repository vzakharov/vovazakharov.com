...
otherwise) and landing in two different places, because only one of them is
worth keeping:

  <slug>.transcript.md    under `--out-dir`, default
                          `docs/remove-before-merging/deepgram/` — one line per
                          sentence with its timecode, plus the words Deepgram
                          was least sure of. This is the file a person or an
                          agent reads, and the one that is committed.
  <slug>.deepgram.json    under `--json-dir`, default `tmp/deepgram/`, which is
                          gitignored — the whole response, with the per-word
                          timings a subtitle track would need. Re-reading a
                          recording costs one API call, which is cheaper than
                          carrying megabytes of JSON in the tree for the one
                          case in a hundred that wants them.
...
DEFAULT_JSON_DIR = Path("tmp") / "deepgram"
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    json_path = args.json_dir / f"{slug}.deepgram.json"
