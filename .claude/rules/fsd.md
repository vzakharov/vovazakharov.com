...
- **Deleting or renaming a route strands a validator on the operator's machine.** `tsconfig.json` globs `.next/dev/types/**`, which only `next dev` writes — one entry per route it saw — and no build prunes it, so `tsc` goes on asserting against a route file the branch removed. The tell is a type error naming a path the repo no longer contains; the fix is `rm -rf .next/dev`, and it belongs in the handover rather than waiting to be hit. A green build here proves nothing about it: this container never runs the dev server, and CI checks out fresh.
