...
- **Two suffixed barrels join `index.ts` as legal entry points**, on two axes that are orthogonal to each other, and the list is closed at those three names (`PUBLIC_API` in `eslint.config.ts`). The ordinary barrel keeps the majority surface in both cases, so a consumer that needs neither suffix never learns they exist.
...
- **A module whose direct import is the hazard says so in its name.** `shared/config/site.env.unsafe.ts` reads `NEXT_PUBLIC_SITE` and parses it, carrying no `server-only` of its own because both barrels above consume it — so nothing but the name stops a client chain importing it and paying zod's ~90 kB. The `.env.unsafe.` suffix is `Playgramai/playgramapp`'s, whose barrel conventions this repo's split follows.
- **`shared/lib` has no root barrel.** It is addressed one sub-library at a time (`@/shared/lib/class-names`), each with its own `index.ts`. It is the holding area, not the destination: a sub-library becomes a top-level segment (`shared/content`) once it has several consumers and a purpose identity of its own, and only a helper too small to name one — `class-names` is a single function — stays under `lib`.
