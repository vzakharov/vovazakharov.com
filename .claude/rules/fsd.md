...
- **`index.server-only.ts` is the one other legal entry point.** An element that holds something a client bundle must not reach exposes it there instead, and every module behind it opens with `import 'server-only'` — the barrel names the split, that import enforces it. The list is closed at those two names (`PUBLIC_API` in `eslint.config.ts`), so a third suffix is still reaching into internals. `shared/i18n` is the case: `routing` sits in the ordinary barrel because client components need the locale list, while `localeSchema` would drag zod's ~90 kB into every chunk that touched it.
