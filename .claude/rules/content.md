...
**That seam now collides with the cut's route form.** A cut is `<slug>.<variant>` in both the file name and the route, so `<slug>.<locale>` would be a second meaning for the same dotted suffix, and a locale that shares a name with a variant would be unresolvable. Whoever builds the localized route decides how the two suffixes coexist — an order (`<slug>.<variant>.<locale>`), a separate namespace, or a locale segment after all. Recording the collision is the whole of what is settled here.
