...
- **A mapped component lives beside the map, not beside the plugin that emits its tag.** The component renders page composition — `ContentVideo` is a document's own UI — so it and the map belong in the `pages` layer (`pages/documents/ui/`). `shared/content` is a layer below, and the one-way import direction forbids it from reaching up into `pages`: the plugin there names the tag, and the map one layer up binds it to the component.
