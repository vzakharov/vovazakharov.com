...
1. Write `apps/<site>/public/<collection>/<slug>.md` with frontmatter:
   ...
   **A line's note is a footnote in its lyrics section** — `[^label]` at the end of the line, `[^label]: …` on a line of its own below the verse, one line of markdown. GitHub renders a footnote as one, which is why it is not a syntax of the site's own; labels are unique across the file for the same reason, so the two languages' notes are suffixed `-ru` and `-en`. A note is in the language of the text it hangs off, and the page shows only the notes of the column in its own language — the crib's on a page whose language is not the one sung, the words' own on a page in it. A marker with no definition, a definition no line carries, and two notes on one line each fail the build.
