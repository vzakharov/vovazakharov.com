...
- **The last three read the agent infrastructure itself and write nothing, which is why they overlap the rest safely.** `scripts/check-skill-catalog.sh` asserts that every `@`-reference into `.claude/` resolves — the skill pointers and CLAUDE.md's own imports alike — and that every section citation into CLAUDE.md names a heading that exists; the failures it catches are silent, so leaving it to be run by hand puts it back on memory, which is where it was when it went unrun. `scripts/staged.sh check` holds `.claude/staged/` to `.claude/rules/staging.md`, and the context budget's tests run the hook under `.claude/context-budget/` against temporary transcripts of their own.
