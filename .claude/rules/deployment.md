...
- **A scope that names no site publishes every site**, as an unscoped subject does: a change to the shared `src/` is every site's change, so that is the safe default and naming a site is the narrowing.
