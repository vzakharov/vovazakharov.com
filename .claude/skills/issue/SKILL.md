...
**One attachment failure is expected here and is not a reason to stop: `403 Forbidden` on a `github.com/user-attachments/…` URL in a web/remote session.** That 403 comes from the agent proxy, not GitHub — it admits only repository-scoped paths on `github.com`, and the attachment host is not one, so no token makes it work. Since GitHub serves everything uploaded through the issue composer from that host, an issue's screenshots and recordings are simply unreachable from a remote session; `private-user-images.githubusercontent.com` and `user-images.githubusercontent.com` are not blocked, so older attachments still download. Tell the 403s apart by their body: the proxy answers with a JSON `message` about repository-scoped endpoints, where GitHub answers with its own error. Proceed on the exported text — it is complete — and **say in your turn which attachments you could not open**, so nobody reads your plan as having accounted for pixels you never saw. If the change hinges on them, ask the operator to paste the image into the session or drop it under `tmp/`; a local-CLI session sits behind no such proxy and fetches everything.
...
How you handle an issue that **does** meet the bar depends on how the session was launched — **plan mode** vs **direct mode**. Here, "plan mode" means `@.claude/skills/plan/SKILL.md`: a web/remote session writes its plan to a `docs/plans/` file and asks its questions as numbered prose rather than through the native plan-mode and `AskUserQuestion` UIs (CLAUDE.md § "Plan mode & questions in web sessions"). So a fresh `/issue` run in a web session is a plan-mode run unless the operator said "no plan"; "direct mode" is the local-CLI case and the explicit waiver.
