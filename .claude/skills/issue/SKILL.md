...
**One attachment failure is expected and is not a reason to stop: `403 Forbidden` on a `github.com/user-attachments/…` URL in a web/remote session.** The agent proxy admits only repository-scoped paths on `github.com`, and that path is not one, so it answers identically with the token, with a different token, and with no auth header at all — the exporter is not doing anything wrong and no header change fixes it. That host is where GitHub puts everything uploaded through the issue composer, so a modern issue's screenshots and recordings are unreachable from a remote session. (`private-user-images.githubusercontent.com` and `user-images.githubusercontent.com` are not blocked; older attachments still download.) The proxy's 403 carries a JSON `message` about repository-scoped endpoints, which is what distinguishes it from GitHub's own. Proceed on the exported text — it is complete — and **say which attachments you could not open**, so no one reads your plan as having accounted for pixels you never saw. If the work hinges on them, ask the operator to paste the image into the session or drop it under `tmp/`; a local-CLI session is behind no such proxy.
