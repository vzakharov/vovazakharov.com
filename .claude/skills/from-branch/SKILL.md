...
**What the PR _says_ is read from an export, never from `gh` JSON.** Its body, its conversation and its review threads — each quoting the lines its comment hangs off — come from `python3 scripts/export-github-item.py <NNN>`, which writes `docs/pr/<NNN>/pr.md`; on a `/handle` prompt `.claude/hooks/prompt-handle-pr-export.sh` has normally taken it before the turn. The fetch above asks only for what the export does not carry, deliberately: `body,reviews,comments` would return the same conversation flattened — no thread structure, no resolved state, none of the `(agent)` / `(human)` labels `@.claude/skills/handle/SKILL.md` Step 2 picks its lane on — and an agent holding it has no reason left to open the export.
