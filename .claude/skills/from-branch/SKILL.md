...
**Always look for any PRs on the target branch — open _or_ closed — and skim them before doing anything else.** Even when the target is a plain branch name (not a `#NNN`/URL), the branch usually has PR history that tells you what the work is, what's already been reviewed, and whether an earlier PR was closed/merged/superseded. Run `gh pr list -R <owner>/<repo> --head <branch> --state all --json number,title,state,url,isDraft,baseRefName`, then for each hit `gh pr view <NNN> -R <owner>/<repo> --json state,isDraft,baseRefName,mergeable,mergeStateStatus`. This grounds the rest of the session in the branch's actual history — a merged PR meaning follow-up must restart from the trunk (see the system prompt's merged-PR rule), or a draft that `/finalize` will later land.

**What the PR _says_ is read from an export, never from `gh` JSON.** Its body, its conversation and its review threads — each quoting the lines its comment hangs off — come from `python3 scripts/export-github-item.py <NNN>`, which writes `docs/pr/<NNN>/pr.md`; on a `/handle` prompt `.claude/hooks/prompt-handle-pr-export.sh` has normally taken it before the turn. The fields above are the ones the export does not carry, which is the whole reason they are fetched here. Adding `body,reviews,comments` to them returns the same conversation with the thread structure, the resolved state and the `(agent)` / `(human)` labels flattened out of it — and an agent holding that has no reason left to open the export, so the read that picks `@.claude/skills/handle/SKILL.md` Step 2's lane gets made from the one source that cannot support it.
...
**A branch carries its own copy of the loop.** Skills, rules and hooks are read from the working tree, so a branch cut before a `.claude/` change runs the loop as it stood then, and nothing about running it feels any different. Check what the trunk has that the branch does not, and name it in the attach report:

```bash
git fetch origin <trunk> && git diff --stat HEAD...origin/<trunk> -- .claude CLAUDE.md
```

A non-empty result is context rather than work — the base merge that clears it is `/finalize` Step 2's, and Step 6's follow-up runs regardless, the same standing the merge state has above. What it changes is what you can trust about your own instructions: a rule the branch's copy does not carry is one this session will not follow, and the operator watching it not happen has no way to see why from the outside.
