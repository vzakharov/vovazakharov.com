...
Nothing runs on pull requests. `./scripts/vet.sh` runs the same builds the deploy does, so a green vet locally is the only pre-merge signal there is — bar the render above, which a deploy runs and no local check stands in for.
