...
- **`scripts/check-notes-length.sh` holds each `writing/notes/` file to the 400-line ceiling its own "How this file is kept" states.** Past it a notes file is squeezed rather than grown, and the squeeze is a judgement — so the check reports and stops rather than trimming. It exists because the ceiling was prose in the file it governs, and nothing reads a paragraph on its way to appending a section. POSIX `sh` and `wc`, reading only `writing/notes/`, which nothing else here touches.
