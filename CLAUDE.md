...
**Editing that file is a subagent's, not this session's** — the entry, and any squeeze `scripts/check-notes-length.sh` asks for. What costs is the loop rather than the one edit — cut, re-measure, cut again, and a placement judged against a file kept as abstraction rather than a log — and run inline, that loop runs inside whichever session happened to trip the ceiling. So hand over the learning in the session's own words — what the agent had settled, what the comment changed — and take back one line saying where it landed, once the check passes. Nothing else goes in the prompt: that file's own "How this file is kept" is the brief. In doubt hand it over anyway, since ruling that a comment was not a bump is the subagent's call too.
