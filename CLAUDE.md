...
- **`costs/`** — What the work here would cost at Claude API rates: `prices.json`, the hand-maintained rate table, and `sessions/<YYYY-MM>/<session-id>.json`, one row per session, written by a `Stop` hook and carried to the trunk by the branch's merge. `pnpm costs` totals them. `@.claude/rules/costs.md` carries how a transcript is priced and how that hook shares the event with the harness's own.
