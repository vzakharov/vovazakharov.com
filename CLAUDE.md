...
The second is **each custom domain's DNS**, which no file here can write and no token here can set, and without which the site is dark whatever CI reports. Each receiving site's apex and `www` records are the operator's to hold; `scripts/publish-site.sh` and each site's own `CNAME` are the whole of what this repository can say about where it is served.
