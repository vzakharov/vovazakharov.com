...
- **`apps/`** — One directory per site — `vova/` and `lsa/` — each holding its `app/` (**routing only**, not the FSD app layer, which is `src/app`), its `public/`, its `next.config.ts` and its `tsconfig.json`. Both build from the one `src/`, and a build is entered in its app directory rather than pointed at from the root.
- **`apps/*/public/`** — Static assets served at that site's root, including `.nojekyll` (required — GitHub Pages otherwise strips Next's `_next/` directory), `apps/lsa/public/CNAME` (the custom domain, which a force-push would otherwise drop), and one directory per content collection (`case-studies/`), holding the authored markdown at the same path its page is routed to.
...
- **`scripts/`** — Agent-facing shell/Python tooling; `vet.sh` is the entrypoint below. `type-overlap-check.ts` (`type-overlap-check.README.md` is its reference) and `render-og.ts` run under `tsx`, the latter because the CV card reads the message catalogue through the app's own merge; `render-pdf.ts` runs under it too, as does `check-i18n-payload.ts`, which reads the locale list off the app rather than restating it; `render-mermaid.ts`, `generate-styles.ts` and `check-mantine-styles.ts` run under bare Node's type stripping.
...
Merging to `main` triggers `.github/workflows/deploy.yml`, which builds both sites' static exports and publishes each to its own domain. **There is no separate release step** — merge _is_ deploy, which is why `/release` and `/hotfix` are not part of this project's skill set.
...
Nothing runs on pull requests. `./scripts/vet.sh` runs the same two builds the deploy does, so a green vet locally is the only pre-merge signal there is.
...
```bash
pnpm build            # both sites' next build — the whole-app end-to-end check
...
pnpm check:i18n-payload   # which pages ship next-intl    │
pnpm content:og --check   # social cards, hashes only    │
...
Fourteen things about that list are deliberate:

- **`pnpm build` is the only check that covers the app itself.** The suite reaches one script so far (see "Testing"), so the static-export build is what catches a broken page, route or import. It is also exactly what CI runs on `main`, so a green vet means a green deploy. It is **two** builds, one per site, run in sequence and neither cached — so a change to either site's text pays for both. That is the accepted cost of two sites out of one repository, and the first thing to revisit if the run becomes annoying.
...
- **`pnpm check:i18n-payload` holds next-intl's client runtime to the pages that render in more than one language**, which is worth 14 kB gzipped on every page that renders in one. It reads the build for the same reason the Mantine check does, and for one more: the permitted set is **derived, not listed** — a locale from `routing.locales` appearing in a page's route or under it is what qualifies the page, so a newly localized page needs no allowlist extended. A lint rule cannot do this, because every legitimate `useTranslations` call is a client component inside a localized page and a path glob is the only discriminator available to it. The marker list that fingerprints the runtime in a minified chunk is the fragile part, so a localized page that ships **no** runtime fails too — markers that stop matching a future next-intl surface as a failure rather than as a check that passes on everything. `.claude/rules/i18n.md` is the home of the convention.
...
- **`scripts/check-skill-catalog.sh` reads the agent infrastructure itself, which is why it overlaps the rest safely** — nothing else here touches `.claude/`. It asserts that every `@`-reference into `.claude/` resolves — the skill pointers and this file's own imports alike; the failure it catches is silent, so leaving it to be run by hand puts it back on memory, which is where it was when it went unrun. What makes it a member is that it measures **this repo's own** skills, which is the test for the whole list — `scripts/gh_export/` is vendored byte for byte and edited only at the source, so its suite runs there. `@.claude/skills/update-muthur/watermark.json` carries that call, under `scripts/check-muthur.sh` and `scripts/test_*.py`.
...
- **`/stand-up-site`** — put a site on its own custom domain: the receiving repository and its deploy key, the DNS records the operator writes, the pre-merge publish that names one site, and settling Pages once the branch has pushed. The half of a new site that leaves no file behind.
- **`/dictation`** — turn a recording into a transcript file under `writing/<project>/dictations/`. `scripts/transcribe.py` is the deterministic half (audio extraction, Deepgram, the saved response); the skill is the half that decides which words in the recognizer's output are the speaker's and which are its mistakes.
- **`/subtitles`** — burn the agreed words onto the video: the corrected transcript joined back onto the saved response's timings, rendered as an HTML page the operator approves, then walked frame by frame into a new video. Specified in full, built not at all.
- **`/dictation-to-post`** — the other thing a recording becomes: the same argument as written text, going by the transcript but free to supply a link the speaker left unsaid, with every departure listed in the draft for the operator to take back out.
- **`/afterword`** — the agent's own reading of a piece, written into the file itself rather than into a thread, in the material's language. The second voice of the two-voice post format, and what a dictation closes with; `/dictation` calls it rather than restating it.
