...
- **`apps/*/public/`** — Static assets served at that site's root, including `.nojekyll` (required — GitHub Pages otherwise strips Next's `_next/` directory), `apps/lsa/public/CNAME` (the custom domain, which a force-push would otherwise drop), and one directory per content collection the site serves (`apps/vova/public/case-studies/`, `apps/lsa/public/bible/`), holding the authored markdown at the same path its page is routed to.
...
- **`costs/`** — What the work here would cost at Claude API rates: `prices.json`, the hand-maintained rate table, and `sessions/<YYYY-MM>/<session-id>.json`, one row per session, written by a `Stop` hook and carried to the trunk by the branch's merge. `totals.json` beside them is those rows summed by month, week and day — derived, so a conflict on it is settled by rerunning `pnpm costs --write` rather than by merging. `@.claude/rules/costs.md` carries how a transcript is priced and how that hook shares the event with the harness's own.
- **`scripts/`** — Agent-facing shell/Python tooling; `vet.sh` is the entrypoint below. `type-overlap-check.ts` (`type-overlap-check.README.md` is its reference) and `render-og.ts` run under `tsx`, the latter because the CV card reads the message catalogue through the app's own merge; `render-pdf.ts` runs under it too; `render-mermaid.ts` and `generate-styles.ts` run under bare Node's type stripping. Every per-site entry in `package.json` goes through `in-site.sh`, which pairs the app directory with `NEXT_PUBLIC_SITE` so no entry spells either out — its header carries the argument form.
...
pnpm content:pdf:vova --check   # PDFs, hashes only     │
pnpm content:pdf:lsa --check    # the other site's       │
...
- **The render checks hash files and nothing else**, so they need no browser and overlap the rest safely. `content:og --check` compares each social card's source — a chart's authored SVG, or the page the CV card is generated from — against `og-renders.json`; `content:pdf:<site> --check` compares each document's whole source set — the markdown, its assets, the print stylesheet and the article components — against `pdf-renders.json`. All of them fail rather than render, because rendering is a committed, run-by-hand step (`.claude/rules/content.md` carries why). The PDF check is one entry per site rather than one script running both: a passed `--check` lands at the end of the command line, so a combined `a && b` would leave the first site rendering for real inside a vet run.
