...
pnpm check:i18n-payload   # no page ships next-intl       │
...
- **`pnpm test` is Node's own runner, loaded through `tsx`** — every `.test.ts` in the tree, no framework installed and none needed. It covers `scripts/type-overlap-check.ts` and `src/shared/i18n/message-markdown.tsx` today; see "Testing" for what belongs in it next.
