...
- **`/feedback`** — the agent's own reading of a piece, posted as a review on the PR that carries it: the whole-piece blocks in the review body, the rest anchored to the passages they are about. Posting ends it — the blocks are the operator's to answer, and what they answer comes back through `/handle`. `/dictation` calls it rather than restating it.
