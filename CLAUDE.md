...
Nothing runs on pull requests. `./scripts/vet.sh` runs the same builds the deploy does, so a green vet locally is the only pre-merge signal there is.
...
Fifteen things about that list are deliberate:
...
- **`pnpm knip` fails on an unused file, export, type or dependency** — what `tsc`'s `noUnusedLocals` sees within a module, held between modules. `knip.ts` names how the repo is entered, so a false "unused file" is an entry missing there, fixed there rather than by exempting the file; `ignoreDependencies` is the one exemption list, each entry carrying its reason. An `export` read only by its own file is a finding too: drop the `export`. It reads source and `package.json` only, which is why it overlaps the others safely.
