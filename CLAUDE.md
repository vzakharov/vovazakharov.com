...
| `styles/`  | The Sass partials shared across `src/`: `_mantine.scss` (counterparts to `postcss-preset-mantine`'s mixins) and `_tokens.scss` (the colour-token mixin). They sit here because a Sass partial is not an FSD module — it has no import graph for the layer checkers to reason about, and a `shared/` segment holding one would need a public API that nothing could import. |
