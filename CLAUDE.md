...
| `scripts/` | Agent-facing shell/Python tooling; `vet.sh` is the entrypoint below. `type-overlap-check.ts` is the one script run under `tsx` — `type-overlap-check.README.md` is its reference; `render-mermaid.ts` and `generate-breakpoints.ts` run under bare Node's type stripping.                                                                                                                                                                                                                     |
...
pnpm styles:breakpoints --check  # generated partial     │
...
- **`pnpm styles:breakpoints --check` holds a generated file to its source.** The breakpoint scale exists in TypeScript for Mantine and in Sass for media queries, which cannot read a custom property; `styles/_breakpoints.scss` is generated from `src/app/styles/breakpoints.ts` so the Sass half is derived rather than remembered, and the check fails on a partial that is stale or hand-edited. `--check` is the read-only form, so it overlaps the rest safely.
