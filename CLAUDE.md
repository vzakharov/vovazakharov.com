...
**Editing that file is a subagent's, not this session's** — the entry, and any squeeze `scripts/check-notes-length.sh` asks for. Both are real editorial work: placing an entry means reading a file kept as abstracted learnings rather than a log, and a squeeze past the ceiling is a hundred lines of it. Done inline it lands on whichever session happens to trip the ceiling, which charges a review that added one line for a hundred. The prompt is the learning in the session's own words — what the agent had settled, what the comment changed — plus _place it, squeeze if the check trips, come back when it passes_. Nothing else: that file's own "How this file is kept" is the brief, and carries what doesn't count as a bump. What comes back is one line saying where the entry landed — reported, not re-checked, since re-reading the file is the cost being avoided.
