...
- **`pnpm test` is Node's own runner, loaded through `tsx`** — every `.test.ts` in the tree, no framework installed and none needed. It covers `scripts/type-overlap-check.ts` and `src/shared/i18n/rich-text.tsx` today; see "Testing" for what belongs in it next.
